...
def create_professor_edit_activity_view(parent_frame, user_data):
    ...
    combo_materias = ttk.Combobox(selection_frame, textvariable=materia_seleccionada_var, 
                                  values=list(materia_options.keys()), state='readonly', width=30)
    ...
    combo_actividades = ttk.Combobox(selection_frame, textvariable=actividad_seleccionada_var, 
    ...
    def cargar_actividades(*args):
        ...
        for widget in form_frame.winfo_children(): widget.destroy()
        ...
        if actividad_descripciones:
            ...
            cargar_datos_actividad() # Cargar datos de la primera
             ...
             form_frame.pack(pady=10, padx=20, fill='x', anchor='n')
    ...
    def cargar_datos_actividad(*args):
        ...
        for widget in form_frame.winfo_children(): widget.destroy() # Limpiar form
        ...
        def guardar_cambios_actividad():
            ...
            if success:
                ...
                if nueva_descripcion != descripcion_original_guardar:
                     try:
                          # Buscar índice de la nueva descripción
                          new_index = combo_actividades['values'].index(nueva_descripcion)
                          combo_actividades.current(new_index)
                     except ValueError:
                          pass # Si no se encuentra por alguna razón, no hacer nada
                else:
                    # Si no cambió, recargar sus datos por si acaso
        ...
        form_frame.pack(pady=10, padx=20, fill='x', anchor='n') # Mostrar el form
