...
class MateriaWidget(ttk.Frame):
    ...
    def __init__(self, parent, materia_info, bg_color):
        ...
        # Contenedor principal: ancho fijo, no propagamos tamaño
        self.container = ttk.LabelFrame(self, width=self.card_width, style=style_name, labelwidget=tk.Label())
        self.container.pack_propagate(False)  # evita que el contenido cambie el ancho
        self.container.pack(anchor='n', padx=5, pady=5)  # no se expande horizontalmente
    ...
    def toggle_expand(self, event=None):
        if self.is_expanded:
            ...
            max_text_width = self.card_width - 40  # ancho fijo menos padding

            if not calificaciones:
                # Mostrar mensaje centrado si no hay actividades, sin expandir ancho
                ttk.Label(
                    self.details_frame,
                ...
                for nombre_act, calif_dict in calificaciones.items():
                    ...
                    max_text_width_detail = self.card_width - 80
                    ttk.Label(detail_line, text=f"{nombre_act}:", style="CardDetail.TLabel",
                              anchor="w", wraplength=max_text_width_detail).pack(side=tk.LEFT, fill='x', expand=True)
...
def create_student_subjects_view(parent_frame, user_data):
    ...
    ttk.Label(container, text="Materias Inscritas", font=("Helvetica", 16, "bold"), style="TLabel")\
    ...
    # Funciones para scroll multiplataforma
    ...
    # Enlazar scroll al canvas y al frame
