...
# --- VISTAS/PESTAÑAS INDIVIDUALES ---
# (Imported by the main view files as needed, keep imports here clean)
# Examples (ensure these exist if imported by the main views):
...
def logout():
    ...
    # Destruir todos los widgets dentro de la ventana raíz
    for widget in root.winfo_children():
        widget.destroy()
    # Resetear la configuración de grid de la ventana raíz
    root.columnconfigure(0, weight=0); root.columnconfigure(1, weight=0); root.rowconfigure(0, weight=0)
    # Volver a mostrar la página de login
    show_login_page()
...
def show_login_page():
    ...
    global root
    if root is None: # Crear la ventana raíz solo una vez
        root = tk.Tk()
    ...
    # Configurar grid para el layout de login
    ...
    except Exception as e: print(f"Error cargando logo.png: {e}")

    ttk.Label(left_content, text="TECMILENIO", font=("Helvetica", 24, "bold"), background="#28a745", foreground="white").pack(pady=10)
    ttk.Label(left_content, text="Bienvenido", font=("Helvetica", 16), background="#28a745", foreground="white").pack(pady=5)
    ...
    def attempt_login():
        ...
        if user_data:
            # Destruir frames de login ANTES de mostrar la app principal
            left_frame.destroy()
            right_frame.destroy()
            # Resetear grid de root
    ...
    # Hacer la ventana visible después de configurarla
    root.deiconify()

    # Iniciar el mainloop solo si no está ya corriendo
...
def show_main_app(user_data):
    ...
    if role == 'admin':
        ...
        root.geometry("950x700") # Puede requerir ajuste
    else: # alumno
        root.geometry("950x700") # Puede requerir ajuste


    # --- Header Photo Update Function (Definida aquí, usada por vistas específicas) ---
    # Esta función necesita saber qué Label actualizar (puede variar según la vista)
    def update_header_photo():
        target_label = None
        current_user_data = db_manager.get_user_by_id(user_data["id"]) # Get fresh data
        if not current_user_data: return

        # Identificar qué Label de foto está actualmente visible
        # Cada _main_view expone su label del header como una variable global en su módulo
        try:
            if role == 'admin':
             ...
             target_label = None # No está listo aún o no existe

        if target_label is None: return # No hay dónde mostrar

        try:
            image_path = current_user_data["ruta_foto"]
            if not os.path.exists(image_path): image_path = "assets/default_user.png"
            # Usar tamaño diferente para el placeholder del admin
...
if __name__ == "__main__":
    # Asegurar que root se cree antes de llamar a show_login_page
    if root is None:
        root = tk.Tk()
    ...
    # El mainloop ya se maneja dentro de show_login_page si es la primera vez