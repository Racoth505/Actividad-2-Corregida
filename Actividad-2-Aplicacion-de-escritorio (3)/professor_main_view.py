...
def create_professor_main_view(root, user_data, logout_func, update_header_func):
    ...
    # Configurar el tema nativo de macOS
    style = ttk.Style()
    style.theme_use("aqua")  # Usar tema nativo para compatibilidad con modo claro/oscuro

    # Configurar estilo para botones (sin colores explícitos)
    ...
    try:
        ...
        ttk.Label(sidebar_frame, text="[BANNER]", font=("Helvetica", 16, "bold"), background="#28a745").grid(row=0, column=0, pady=20, padx=10)

    # --- Nav Frame ---
    nav_frame = tk.Frame(sidebar_frame, bg="#28a745")
    nav_frame.grid(row=1, column=0, sticky="new", pady=10, padx=10)
    nav_frame.columnconfigure(0, weight=1)  # Una sola columna
    ...
    btn_perfil = ttk.Button(
        ...
        compound=tk.LEFT,
        command=lambda: show_professor_view(content_frame, user_data, "profile")
    )
